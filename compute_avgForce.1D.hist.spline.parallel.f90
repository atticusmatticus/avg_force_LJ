...
module histData
   use prec
   real(kind=dp),allocatable :: histDist(:), histCosTh(:), histPhi(:), g(:,:,:,:), fr(:,:,:,:), fs(:,:,:,:), ft(:,:,:,:), &
      & g2(:,:), fr2(:,:), fs2(:,:), gc(:,:,:,:), gTmp1(:,:), gTmp2(:,:), frTmp1(:,:), fsTmp1(:,:)
...
module cfgData
   ...
   integer :: cfgRBins, cfgCosThBins, cfgPhiBins, cfgPsiBins, radius, nThreads
!
   integer :: xBins, zBins
...
subroutine make_hist_table(histFile)
   ...
   allocate( histTmp(13,nHistLines) )
   ...
   allocate( histDist(histDistBins), histCosTh(histCosThBins), histPhi(histPhiBins), g(2,histDistBins,histCosThBins,histPhiBins), &
      & fr(2,histDistBins,histCosThBins,histPhiBins), fs(2,histDistBins,histCosThBins,histPhiBins), &
      & ft(2,histDistBins,histCosThBins,histPhiBins), gc(2,histDistBins,histCosThBins,histPhiBins) )
   ...
   do i = 1, histDistBins
      do j = 1, histCosThBins
         do k = 1, histPhiBins
            ...
            fr(1,i,j,k) = histTmp( 6, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.r>(r,cos,phi) +
            fs(1,i,j,k) = histTmp( 7, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.s>(r,cos,phi) +
            ft(1,i,j,k) = histTmp( 8, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.t>(r,cos,phi) +
            fr(2,i,j,k) = histTmp( 9, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.r>(r,cos,phi) +
            fs(2,i,j,k) = histTmp(10, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.s>(r,cos,phi) +
            ft(2,i,j,k) = histTmp(11, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! <f.t>(r,cos,phi) +
            gc(1,i,j,k) = histTmp(12, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! gc(r,cos,phi) +
            gc(2,i,j,k) = histTmp(13, (i-1)*histCosThBins*histPhiBins + (j-1)*histPhiBins + k)  ! gc(r,cos,phi) +
         end do
      end do
   end do
...
subroutine spline_hist_array
   ...
   implicit none
   integer                     :: ia, i, ir, ith, iphi, imin, igo, ir2, igo1, igo2
   ...
   ! Allocate arrays for 3D -> 2D transformation
   ...
   ! Allocate arrays for 2D -> 1D transformation
...
find:       do ir = 1, histDistBins
            ...
            do ir = histDistBins, 1, -1
               if (ir.ge.imin) then
                  if (g(ia,ir,ith,iphi).gt.1d-6) then
                     g(ia,ir,ith,iphi) = log(g(ia,ir,ith,iphi))
                     ...
                     fr(ia,ir,ith,iphi) = idealHist(2,ir,ith,iphi)
                     fs(ia,ir,ith,iphi) = idealHist(3,ir,ith,iphi)
                     ft(ia,ir,ith,iphi) = idealHist(4,ir,ith,iphi)
                  end if
                  ...
                  g(ia,ir,ith,iphi) = ( -idealHist(1,ir,ith,iphi) + idealHist(1,imin,ith,iphi) ) + g(ia,imin,ith,iphi)
      ...
      do iphi = 1, histPhiBins
         do ith = 1, histCosThBins
            do ir = histDistBins, 1, -1
               if ((fr(ia,ir,ith,iphi).gt.-cut).and.(igo.eq.0)) then
                  fr(ia,1:ir,ith,iphi) = -cut
                  igo = 1
               end if
               if ((fs(ia,ir,ith,iphi).gt.-cut).and.(igo1.eq.0)) then
                  fs(ia,1:ir,ith,iphi) = -cut
                  igo1 = 1
               end if
               if ((ft(ia,ir,ith,iphi).gt.-cut).and.(igo2.eq.0)) then
                  ft(ia,1:ir,ith,iphi) = -cut
                  igo2 = 1
               end if
            end do
            igo = 0; igo1 = 0; igo2 = 0
      ...
      ! an array u0(r,th).
      u0 = minval(-g(ia,:,:,:),dim=3) !fixme does dim need to be 4 now?

      do ith = 1, histCosThBins
         do ir = 1, histDistBins
            boltz_sum = 0_dp
            do iphi = 1, histPhiBins
               boltz = exp(g(ia,ir,ith,iphi) + u0(ir,ith))
               g2D(ir,ith) = g2D(ir,ith) + exp(g(ia,ir,ith,iphi)) ! g is g
               fr2D(ir,ith) = fr2D(ir,ith) + (boltz * fr(ia,ir,ith,iphi)) ! f.r
               fs2D(ir,ith) = fs2D(ir,ith) + (boltz * fs(ia,ir,ith,iphi)) ! f.s
...
subroutine compute_avg_force
   ...
   do r = 1, cfgRBins ! loop lj--lj distances
      ...
      !$omp SHARED( nThreads, r, xBins, zBins, cut, R_axis, x_axis, z_axis, histDist, histDistBins, g1D, g1D2, fr1D, fr1D2, &
      !$omp&   frcSPA, grSPA ) &
